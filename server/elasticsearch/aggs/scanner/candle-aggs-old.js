 ...
 *
 * */
function write_to_csv (data, file_name) {
 ...
 *
 */
async function get_candles (symbol, interval, from, to) {
	...
	try {
		...
		_.each(es_buckets, function (obj) {
			...
			if (obj.the_high.value) {
				...
				candles.push({
					exchange_name: obj.the_exchange_name.buckets[0].key,
...
async function prev_close_all_markets (interval, percent_rise, percent_drop) {
	...
	try {
		...
		write_to_csv(_.reverse(result), csv_file_name)
...
async function test_init () {
	...
	try {
		...
		write_to_csv(_.reverse(_prev_close), csv_file_name)
